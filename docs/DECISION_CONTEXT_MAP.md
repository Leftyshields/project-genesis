...
When `run_config.json` mode is autonomous, the **Default** line is the decision unless a named conflict rule applies. In interactive mode the same Default is the recommended choice unless the Creator overrides it.

When two governing laws pull opposite directions, name both and resolve with the rule in section 9. **Never average them** or default to the more familiar one.
...
Classify the decision, then open that section only.

| Decision in front of you | Section | Laws |
|--------------------------|---------|------|
| Menus, choice sets, onboarding steps, settings density | 1 | Hick's, Miller's |
| Button size, tap targets, primary-action placement | 2 | Fitts's |
| Spacing, grouping, cards, visual simplicity | 3 | Proximity, Similarity, Connectedness, Prägnanz |
| Emphasis, list order, success/error endings, unfinished tasks | 4 | Von Restorff, Serial Position, Peak-End, Zeigarnik |
| Feedback latency, perceived performance | 5 | Doherty |
| Convention vs novelty; forgiving input | 6 | Jakob's, Postel's |
| Who absorbs inherent complexity | 7 | Tesler's |
| Feature scope, prioritization, pacing (**not** pixels) | 8 | Occam's, Pareto, Parkinson's |
| Two laws pull opposite ways | 9 | Conflict pairs |

If you cannot classify, read **this routing table only**, then stop. Do not load sections 1–9 by default.

Strategy-level heuristics (section 8) belong at `/design_decisions` or `/create_plan`. Do not apply them to component layout.
...
**Doherty threshold.** Productivity rises sharply when the system responds in under 400ms. The only law here with a testable number, so it doubles as a QA checklist check for interactive UI (timed harness is app-repo follow-on). Governs feedback latency and perceived performance. Default: target sub-400ms feedback; where impossible, use optimistic UI, skeleton states, and progressive reveal rather than a bare spinner.
